...
# Load the CSV file with manual column names (generated from rf_meter_logger.py)
column_names = ['timestamp in ms', 'RF power (dBm)', 'RF power (mW)']
data = pd.read_csv(sys.argv[1], names=column_names, usecols=[0, 1, 2])

# If the second argument is provided, use it as a correction value to adjust the RF power readings (in dBm).
if len(sys.argv) > 2:
    try:
...
# Exclude the lower 1% of the data based on 'RF power (dBm)'
lower_bound_dBm = data['RF power (dBm)'].quantile(0.01)
data = data[data['RF power (dBm)'] > lower_bound_dBm]

# Exclude the lower 1% of the data based on 'RF power (mW)'
lower_bound_mW = data['RF power (mW)'].quantile(0.01)
